...
export class WorkflowRepository {
  ...
  async deleteWfvUserParam ({ userId, wfvId, paramName, trx = this.prisma }: { userId: number, wfvId: number, paramName: string, trx?: lib.PrismaLibService }) {
  ...
  async deleteWfvUserParams ({ userId, wfvId, trx = this.prisma }: { userId: number, wfvId: number, trx?: lib.PrismaLibService }) {
    await trx.workflowVariantUserParams.deleteMany({
      where: {
        userId,
        workflowVariantId: wfvId
  ...
  async findWorkflowVariantsByParamName ({ paramName }: { paramName: string }) {
    ...
    const likePattern = `${classType}:${name}:%`

    const workflowVariants = await this.prisma.$queryRaw<WorkflowVariants[]>`
      SELECT DISTINCT wv.*
        FROM workflow_variant_params AS wfvp
       INNER JOIN workflow_variants AS wv
  ...
  async findWorkflowVariantsByLabel ({ label }: { label: string }) {
    const workflowVariants = await this.prisma.$queryRaw<WorkflowVariants[]>`
      SELECT DISTINCT wv.*
        FROM workflow_variant_params AS wfvp
       INNER JOIN workflow_variants AS wv
